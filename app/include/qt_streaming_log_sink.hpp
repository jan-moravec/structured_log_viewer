...
public:
    ...
    /// Number of `StreamLogLine` rows currently held in the paused buffer.
    /// Reflected in the status-bar `K buffered` indicator. GUI thread.
...
private:
    ...
    /// batch. The first batch keeps its `firstLineNumber`; subsequent
    /// batches' rows / errors / newKeys are concatenated. Used by
    /// `Resume()` and `TakePausedBuffer()`.
    static loglib::StreamedBatch CoalesceLocked(std::vector<loglib::StreamedBatch> &&batches);

    /// Number of `StreamLogLine` rows currently in the buffer. Caller
    /// must hold `mPausedMutex`.
