...
| `loglib/tailing_bytes_producer.hpp` | `TailingBytesProducer` is the only concrete `BytesProducer` today: a one-thread tailer that pre-fills the last *N* complete lines and follows growth via [`efsw`](https://github.com/SpartanJ/efsw) with a polling fallback. Survives rename-and-create / copy-truncate / in-place truncate / delete-then-recreate rotations and coalesces bursts within a 1 s window. Future stdin / TCP / UDP / named-pipe producers slot in behind the same hook. |
...
- `QtStreamingLogSink` (`app/include/qt_streaming_log_sink.hpp`) — the bridge that turns a `loglib::LogParseSink` callback running on a TBB / streaming worker thread into a `LogModel::AppendBatch` call on the GUI thread (`Qt::QueuedConnection`). It owns a generation counter so a fresh `Arm()` (or a `RequestStop()`) drops any still-queued batches from a previous parse, and a bounded paused buffer with `Pause` / `Resume` / `SetRetentionCap` / `TakePausedBuffer` for the Stream-Mode toolbar (`Resume` coalesces the buffered batches into a single queued post). `PausedDropCount()` reports how many lines were FIFO-evicted from the paused buffer for the status bar's `… , X dropped while paused` suffix.
- `StreamOrderProxyModel` (`app/include/stream_order_proxy_model.hpp`) — optional newest-first reversal layer between `LogModel` and `LogFilterModel`. Sorts on `LogModelItemDataRole::InsertionOrderRole` so the user's column sort on the downstream `LogFilterModel` never overlaps. Driven exclusively from `MainWindow::ApplyStreamingDisplayOrder`, which keeps the proxy direction, the `LogTableView` tail edge, and the alternating-row-colours flag in lockstep.
- `LogFilterModel` (`app/include/log_filter_model.hpp`) — the `QSortFilterProxyModel` over `StreamOrderProxyModel` that implements the multi-column filter set described in the [user guide](doc/README.md#filtering). The view chain is therefore `LogModel → StreamOrderProxyModel → LogFilterModel → LogTableView`.
...
- `StreamingControl` (`app/include/streaming_control.hpp`) — `QSettings`-backed transactional store for the **Streaming** group of `PreferencesEditor` (retention cap, newest-first flag). Same Ok / Cancel pattern as `AppearanceControl`: in-memory mutation, `SaveConfiguration` commits, `LoadConfiguration` reverts.
- `PreferencesEditor` (`app/include/preferences_editor.hpp`) — Qt widget hosting the **Appearance** and **Streaming** groups; emits `streamingRetentionChanged` / `streamingDisplayOrderChanged` on Ok so `MainWindow` can re-apply them live.
...
| **Coalescing target** | `kStaticBatchFlushLines = 1000` lines or `50 ms` (throughput optimised).                                                          | `kStreamingBatchFlushLines = 250` lines or `100 ms` (latency optimised).                                                                                                                                    |
  ...
  └─────────────┬──────────────┘         └─────────────────┬────────────────┘
          ...
          (loglib::ParseFile)               (Qt::QueuedConnection,
                                  ...
                                  + dataChanged on back-filled cells
                                                      │
                                                      ▼
                                  StreamOrderProxyModel (optional newest-first)
                                                      │
                                                      ▼
...
1. **A sink consumes the batches.** Two `LogParseSink` implementations ship today:
   ...
   - `QtStreamingLogSink` is the GUI bridge. Its `OnBatch(StreamedBatch)` posts a `QMetaObject::invokeMethod` lambda back to the GUI thread, which drops on a generation mismatch (a fresh `Arm()` or `RequestStop()` invalidates queued batches from a previous parse) and otherwise calls `LogModel::AppendBatch`. While **Pause** is engaged, the sink instead appends batches to a bounded paused buffer (FIFO-trimmed against the retention cap, with `PausedDropCount()` tracking the loss); **Resume** drains the buffer in a single coalesced post.
...
1. **The view renders the rows.** `StreamOrderProxyModel` optionally reverses the order for newest-first display (sorting on `LogModelItemDataRole::InsertionOrderRole`), `LogFilterModel` (`QSortFilterProxyModel`) applies the active filters, and `LogTableView` renders the surviving rows. Follow-newest scrolls land on the proxy-mapped index of the most-recently-appended source row; the view's `TailEdge` (set from `MainWindow::ApplyStreamingDisplayOrder`) keeps the auto-disengage / re-engage signals on the right scrollbar edge. The `Edit → Copy` shortcut pulls the original log text via `LogLine::Source()->RawLine(lineId)` so the row round-trips back to its on-disk (or on-tail) format.

A few cross-cutting invariants hold this together:

- **Each `LineSource` outlives every `LogLine` that quotes it.** That is why `LogModel::Reset()` / `LogModel::StopAndKeepRows()` and `~LogModel()` run a strict teardown order — `BytesProducer::Stop()` first (releases the live-tail worker parked on I/O), then sink `RequestStop()`, then `mStreamingWatcher` join, then paused-buffer flush, then `DropPendingBatches()` — before tearing down the `LogTable`. The cooperative `stop_token` only halts the Stage A token loop / the streaming-loop top, but tasks already in flight keep reading from the source.
...
Most third-party C++ dependencies (`date`, `fmt`, `Catch2`, `mio`, `glaze`, `simdjson`, `robin-map`, `efsw`) are fetched automatically via `FetchContent`. To use system copies instead, pass the corresponding option when configuring — one of `USE_SYSTEM_DATE`, `USE_SYSTEM_FMT`, `USE_SYSTEM_CATCH2`, `USE_SYSTEM_MIO`, `USE_SYSTEM_GLAZE`, `USE_SYSTEM_SIMDJSON`, `USE_SYSTEM_ROBIN_MAP`, `USE_SYSTEM_EFSW` (e.g. `-DUSE_SYSTEM_FMT=ON`). See [`cmake/FetchDependencies.cmake`](cmake/FetchDependencies.cmake) for the pinned versions.
