...
| `loglib/log_table.hpp`              | `LogTable` pairs `LogData` with a `LogConfigurationManager`, owns the `BeginStreaming`/`AppendBatch` state machine, back-fills timestamps mid-stream, drives per-column `EnumCandidateTracker`s + `EnumDictionaryRegistry`, and exposes `EvictPrefixRows(count)`. `mIsStreaming` switches auto-detection between **stream-mode** (promote at 2 rows, no cardinality bail) and **static-mode** (4096 rows + cardinality bail; smaller files are caught by `FinalizeAutoDetection`). `FinalizeAutoDetection()` runs a permissive end-of-parse sweep (`presenceCount >= 2`) so small or slow logs still get enum UI. The default `EnumValueCap` of 64 dominates the cardinality bail at the static threshold; the ratio (`0.05`) only governs sparse-presence candidates that stay below cap. `ResolveEnumColumn(columnIndex)` is the canonical seam GUI predicates / sort caches use to translate a visible column into a `KeyId` + `EnumDictionary*` pair. After a column promotes to `Type::Enumeration`, `MaybePromoteToLevel` runs a second-step check: if the column's key matches `IsLogLevelKey` (`level`, `severity`, `loglevel`, ...) AND at least 80% of dictionary entries resolve to a canonical `LogLevel` via `ResolveLevel` (built-in alias table + per-column `levelMapping` overrides), the type flips to `Type::Level` and `mLevelRankCache[column.header]` is populated with the `EnumValueId -> LogLevel` mapping. `GetLevelForRow(row, columnIndex)` is the public accessor used by sort (`CompareLevel`), filter (`MainWindow::BuildRowPredicates`), and any future row-styling code.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
