...
| `loglib/enum_dictionary.hpp`        | `EnumDictionary` is a per-column intern table for distinct string values (insertion-ordered, `EnumValueId` is `uint16_t`); `EnumDictionaryRegistry` maps `KeyId` → `EnumDictionary` for every column promoted to `Type::enumeration`, with `Alias` keeping multi-key columns (`["level", "severity"]`) sharing one dictionary. `LogTable` owns the registry and points every `LineSource` at it so `CompactLogValue::Materialise(DictRef)` can resolve bytes. Single-writer (the `LogTable` thread) / multiple-reader (anyone resolving a `DictRef`).                                                                   |
...
1. **`LogModel::AppendBatch` updates the table.** With a non-zero `RetentionCap()` (Stream Mode), the visible row prefix is FIFO-evicted before insertion and over-cap batches are head-collapsed first so per-batch eviction stays O(cap). The batch is then handed to `LogTable::AppendBatch`, which:
   ...
   - feeds every value of every `Type::unknown` column into the per-column `EnumCandidateTracker`s. The tracker remembers distinct strings (capped at the runtime `EnumValueCap()`, default `64`), enforces a `MAX_ENUM_CANDIDATE_LEN` cap at observation time, and (via a single `LogLine::FindCompact` walk per row that dispatches on the slot's `CompactTag`) also counts `intObservations` / `doubleObservations` for the no-string bail path. Promotion is mode-aware: in **stream mode** (`mIsStreaming == true`) any candidate that has seen at least `STREAM_PROMOTION_MIN_ROWS = 2` rows and is still under cap flips to `Type::enumeration` immediately, and the cardinality-ratio bail is disabled so a slow vocabulary still gets the enum UI; in **static mode** the threshold is `16` rows for `WELL_KNOWN_ENUM_KEYS` (`level`, …) and `256` rows for everything else, plus the cardinality bail. On promotion an `EnumDictionary` is allocated in the registry, every existing row's slot for that column is re-encoded in place to a `DictRef`, and `enumColumnsChanged` is queued for emission. Bail paths route to a semantically meaningful terminal type instead of `Type::any`: a candidate killed by `MAX_ENUM_CANDIDATE_LEN`, the cardinality bail, or the `EnumValueCap()` overflow becomes `Type::string`; a no-string candidate becomes `Type::integer` (only ints seen), `Type::floating` (only doubles), `Type::number` (mixed), or `Type::any` (no values observed yet — e.g. bool / monostate columns). The terminal type itself prevents re-scanning, so a column that briefly looked enum-like never oscillates within the session,
