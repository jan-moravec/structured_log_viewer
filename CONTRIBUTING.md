...
- `LogFilterModel` (`app/include/log_filter_model.hpp`) — the `QSortFilterProxyModel` over `RowOrderProxyModel` that implements the multi-column filter set described in the [user guide](doc/README.md#filtering). `filterAcceptsRow` consults a vector of `loglib::RowPredicate` directly against the underlying `loglib::LogTable`, skipping the per-row `data(SortRole)` → `QVariant<QString>` round-trip. `lessThan` dispatches to `loglib::CompareRows` with a per-column `EnumDictRank` cache for `Type::Enumeration` columns. The view chain is therefore `LogModel → RowOrderProxyModel → LogFilterModel → LogTableView`. The concrete predicates live in `library/include/loglib/log_filter.hpp` as a closed `std::variant<EnumRowPredicate, TimeRangeRowPredicate, CallbackStringRowPredicate>`: `loglib::CallbackStringRowPredicate` (exactly / contains / regex / wildcard via a caller-supplied callback so Qt's regex flavours stay app-side; `MainWindow::MakeStringMatcher` pre-compiles the regex once at submission and the matcher captures it by value), `loglib::TimeRangeRowPredicate` (begin/end inclusive range; an inverted range rejects every row), and `loglib::EnumRowPredicate` (multi-select equality on `Type::Enumeration` columns; pre-resolves selected strings to a `vector<bool>` indexed by `EnumValueId` for the fast path and falls back to a transparent-hash `unordered_set<string>` for rows whose slot is not yet a `DictRef`). A free `loglib::MatchesRow(predicate, table, row)` overload `std::visit`s to the concrete `MatchesRow` member at compile time, so the per-row hot path pays neither a heap allocation nor a virtual dispatch. The comparator and rank live in `library/include/loglib/log_compare.hpp`.
