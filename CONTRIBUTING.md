...
| `loglib/line_source.hpp`            | `LineSource` is the polymorphic seam every `LogLine` carries (paired with a `lineId`). It owns the bytes backing each line and resolves `CompactTag::MmapSlice` / `CompactTag::OwnedString` payloads through `ResolveMmapBytes` / `ResolveOwnedBytes`. It also carries a borrowed pointer to the session's `EnumDictionaryRegistry` (`EnumDictionaries()` / `SetEnumDictionaries()`) so `CompactTag::DictRef` payloads can resolve to bytes. `BytesAreStable()` discriminates mmap-backed sources from streaming ones; `SupportsEviction()` / `EvictBefore` are the retention hook used by `LogTable::EvictPrefixRows`. |
...
| `loglib/enum_dictionary.hpp`        | `EnumDictionary` is a per-column intern table for distinct string values (insertion-ordered, `EnumValueId` is `uint16_t`). Values live in a `std::deque<std::string>` so each interned string's address is stable; the index `tsl::robin_map<std::string_view, EnumValueId, ...>` keys against those stable bytes (one source of truth per value, ~50% off the dictionary heap vs. the old vector + `robin_map<std::string,...>` pair). `EnumDictionaryRegistry` maps `KeyId` → `EnumDictionary` for every column promoted to `Type::enumeration`; each canonical entry tracks its alias `KeyId`s in a small vector so `Erase` walks only that column's keys. `Alias(canonical, alias)` returns `[[nodiscard]] bool` — callers must check for canonical-missing / already-aliased configuration errors. `LogTable` owns the registry and points every `LineSource` at it so `CompactLogValue::Materialise(DictRef)` can resolve bytes. Single-writer (the `LogTable` thread) / multiple-reader (anyone resolving a `DictRef`).                                                                                                                                                                   |
| `loglib/log_line.hpp`               | `LogLine` holds one parsed record as a sorted vector of `(KeyId, internal::CompactLogValue)` pairs plus a `(LineSource *, lineId)` pair. `CompactLogValue` is a 16-byte union (mmap slice / owned-string offset / int / uint / double / bool / timestamp / monostate / `DictRef` for enum slots); the `LineSource` resolves the row's raw bytes regardless of whether they came from an mmap, a live byte producer, or an `EnumDictionary`.                                                                                                                                                                             |
...
| `loglib/log_configuration.hpp`      | `LogConfiguration` lists the visible columns (header, JSON keys, print format, `Type`, time-parse formats, filters with `LogFilter::Type::text` / `time` / `enumeration` and a `filterValues` enum-picker list). `Type` has one **candidate** state (`unknown`, default for newly observed columns — the auto-detector in `LogTable` scans these) and seven terminal states (`any`, `string`, `integer`, `floating`, `number`, `time`, `enumeration`). The type itself is the kill-once-stay-killed gate. `LogConfigurationManager` loads / saves the file and grows the layout via `AppendKeys`.                       |
| `loglib/log_table.hpp`              | `LogTable` pairs `LogData` with a `LogConfigurationManager`, owns the `BeginStreaming`/`AppendBatch` state machine, back-fills timestamps mid-stream, drives per-column `EnumCandidateTracker`s + `EnumDictionaryRegistry` (column `Type` is the kill-once-stay-killed gate), and exposes `EvictPrefixRows(count)`. `mIsStreaming` (set by `BeginStreaming`) switches auto-detection between **stream-mode** (promote at 2 rows, no cardinality bail) and **static-mode** (16 / 256 rows + cardinality bail). `FinalizeAutoDetection()` runs a permissive end-of-parse sweep so small or slow logs still get enum UI.   |
...
- `LogModel` (`app/include/log_model.hpp`) — a `QAbstractTableModel` that owns the `LogTable`, the `QtStreamingLogSink`, the active parser `QFutureWatcher`, and (in Stream Mode) the live `BytesProducer`. Exposes a custom `EnumValueRole` so `EnumFilterRule` can consume the raw `EnumValueId` for a `DictRef` cell, and emits `enumColumnsChanged` whenever a column flips into or out of `Type::enumeration` so `MainWindow` can rebuild the filter editor's column list. Population is always streaming-driven through one of two entry points:
...
- `LogFilterModel` (`app/include/log_filter_model.hpp`) — the `QSortFilterProxyModel` over `RowOrderProxyModel` that implements the multi-column filter set described in the [user guide](doc/README.md#filtering). Per-row `filterAcceptsRow` calls each active `FilterRule::Matches(displayOrSort, enumValueId)`; the `enumValueId` argument is `EnumValueRole`-sourced and only `EnumFilterRule` consumes it. The view chain is therefore `LogModel → RowOrderProxyModel → LogFilterModel → LogTableView`. Concrete rules live in `app/include/filter_rule.hpp`: `TextFilterRule` (exactly / contains / regex / wildcard), `TimeStampFilterRule` (begin/end range), and `EnumFilterRule` (multi-select equality on `Type::enumeration` columns; pre-resolves the selected strings to a `vector<bool>` indexed by `EnumValueId` for the fast path and falls back to `QSet<QString>` matching for rows whose slot is not yet a `DictRef`).
