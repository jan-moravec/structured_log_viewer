...
| `loglib/log_table.hpp`              | `LogTable` pairs `LogData` with a `LogConfigurationManager`, owns the `BeginStreaming`/`AppendBatch` state machine, back-fills timestamps mid-stream, drives per-column `EnumCandidateTracker`s + `EnumDictionaryRegistry`, and exposes `EvictPrefixRows(count)`. `mIsStreaming` switches auto-detection between **stream-mode** (promote at 2 rows, no cardinality bail) and **static-mode** (16 / 256 rows + cardinality bail). `FinalizeAutoDetection()` runs a permissive end-of-parse sweep so small or slow logs still get enum UI.                                                                      |
...
- `LogModel` (`app/include/log_model.hpp`) — a `QAbstractTableModel` that owns the `LogTable`, the `QtStreamingLogSink`, the active parser `QFutureWatcher`, and (in Stream Mode) the live `BytesProducer`. Exposes a custom `EnumValueRole` (still wired for `Qt::DisplayRole` / `SortRole` queries from Qt's internals) and emits `enumColumnsChanged` whenever a column flips into or out of `Type::Enumeration` so `MainWindow` can rebuild the filter editor's column list and `LogFilterModel` can drop its cached enum sort ranks. Population is always streaming-driven through one of two entry points:
...
- `LogFilterModel` (`app/include/log_filter_model.hpp`) — the `QSortFilterProxyModel` over `RowOrderProxyModel` that implements the multi-column filter set described in the [user guide](doc/README.md#filtering). `filterAcceptsRow` consults a vector of `loglib::RowPredicate` directly against the underlying `loglib::LogTable`, skipping the per-row `data(SortRole)` → `QVariant<QString>` round-trip. The rule list is ordered cheapest-first by `MainWindow::UpdateFilters` (`EnumRowPredicate` → `TimeRangeRowPredicate` → `CallbackStringRowPredicate`) so the `std::ranges::all_of` walk short-circuits on the cheapest test that rejects the row. `lessThan` dispatches to `loglib::CompareRows` with a `EnumDictRank` cache keyed by canonical `loglib::KeyId` for `Type::Enumeration` columns; keying by `KeyId` (rather than column index) means the cache survives column reorders without an explicit `columnsMoved` invalidation hook, and `EnumRankFor` self-heals when either the live dictionary grows past the cached size or the `EnumDictionary*` pointer no longer matches (covers demote → re-promote that re-creates the registry entry at the same `Size()`). The view chain is therefore `LogModel → RowOrderProxyModel → LogFilterModel → LogTableView`. The concrete predicates live in `library/include/loglib/log_filter.hpp` as a closed `std::variant<EnumRowPredicate, TimeRangeRowPredicate, CallbackStringRowPredicate>`: `loglib::CallbackStringRowPredicate` (exactly / contains / regex / wildcard via a caller-supplied callback so Qt's regex flavours stay app-side; `MainWindow::MakeStringMatcher` pre-compiles the regex once at submission and the matcher captures it by value, and `MainWindow::FilterSubmitted` probes `QRegularExpression::isValid()` up front so an invalid pattern is rejected with a status-bar message instead of silently hiding every row), `loglib::TimeRangeRowPredicate` (begin/end inclusive range; an inverted range is rejected at submission time in `MainWindow::FilterTimeStampSubmitted` so the predicate never sees a `begin > end` pair), and `loglib::EnumRowPredicate` (multi-select equality on `Type::Enumeration` columns; pre-resolves selected strings to a `vector<bool>` indexed by `EnumValueId` for the fast path and falls back to a transparent-hash `unordered_set<string>` for rows whose slot is not yet a `DictRef`). A free `loglib::MatchesRow(predicate, table, row)` overload `std::visit`s to the concrete `MatchesRow` member at compile time, so the per-row hot path pays neither a heap allocation nor a virtual dispatch. The comparator and rank live in `library/include/loglib/log_compare.hpp`.

  Sorting behaviour: `loglib::CompareRows` is a total three-way comparator dispatched on the column's logical type. In an ascending sort `std::monostate` and slots not representable in the column's logical type (NaN in an `Integer` column, a stray string slot in a `Floating` column, etc.) share the *tail*: they compare equal to one another and strictly greater than every populated, representable value. The per-type membership of that tail bucket is documented on `CompareRows` in `library/include/loglib/log_compare.hpp` and pinned by regression tests in `test/lib/src/test_log_compare.cpp` (NaN-in-Int and stray-string-in-Floating both tie with monostate). This is a deliberate change from the pre-`CompareRows` behaviour, where invalid `QVariant`s (the prior representation of monostate slots) sorted to the *top* of an ascending sort; if a saved configuration relies on the old order, re-sort by clicking the column header to invert the direction.
