...
| `loglib/log_configuration.hpp`      | `LogConfiguration` describes the visible columns (header, JSON keys, print format, `Type::any` / `Type::time` / `Type::enumeration`, time-parse formats, filters with `LogFilter::Type::text` / `time` / `enumeration` and a `filterValues` list for the enum picker). `LogConfigurationManager` loads/saves it, grows the layout during streaming via `AppendKeys`, and tracks `mAutoDiscoveredCanonicalKeys` so a column the user explicitly carried as `any` is never auto-promoted to `enumeration` later.                                                                                                          |
| `loglib/log_table.hpp`              | `LogTable` pairs a `LogData` with a `LogConfigurationManager` and resolves what to render in cell `(row, column)`. It owns the `BeginStreaming` / `AppendBatch` state machine that the GUI model drives, back-fills timestamp columns mid-stream as new keys appear, runs the enum auto-detection / promotion / demotion loop (per-column `EnumCandidateTracker`s + `mEnumPermanentlyKilled` blacklist + the `EnumDictionaryRegistry` it owns), and exposes `EvictPrefixRows(count)` for the Stream-Mode retention cap.                                                                                                 |
...
1. **`LogModel::AppendBatch` updates the table.** With a non-zero `RetentionCap()` (Stream Mode), the visible row prefix is FIFO-evicted before insertion and over-cap batches are head-collapsed first so per-batch eviction stays O(cap). The batch is then handed to `LogTable::AppendBatch`, which:
   ...
   - feeds every text-shaped value into the per-column `EnumCandidateTracker`s; once a column has been observed for `ENUM_PROMOTION_MIN_ROWS` (256) rows and is still under the runtime `EnumValueCap()` (default 64), the column is promoted to `Type::enumeration`, an `EnumDictionary` is allocated in the registry, every existing row's slot for that column is re-encoded in place to a `DictRef`, and `enumColumnsChanged` is queued for emission. If a 65th distinct value later arrives, the column is demoted back to `any`, every `DictRef` slot is materialised back into `OwnedString`, the column is added to `mEnumPermanentlyKilled`, and the dictionary is dropped — so a column that briefly looked enum-like never oscillates within the session,
