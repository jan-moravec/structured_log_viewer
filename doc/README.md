...
- **Bucket size** — the strip auto-picks between `1 s`, `10 s`, `1 min`, `10 min`, `1 h`, and `1 d` based on the observed time range so ~500 buckets always cover the visible span.
- **Details strip** — a single-line readout below the bars. Idle it shows the current bucket size, total row count, and full time range; hovering a bar switches it to that bar's time span, total, and per-level counts. Replaces the older `QToolTip` hover popup so the readout stays visible without racing pointer motion.
- **Anchor tick strip** — a thin overlay along the top of the plot area. Each column that contains at least one [anchored](#anchoring-and-jumping-to-important-rows) row gets a coloured tick per anchor slot present in that bucket, so anchored spikes stand out even after zooming out. The overlay stays hidden when the session has no anchors, so unrelated logs pay no visual cost. Clicking on a tick jumps to the earliest anchored row inside the column (instead of the bucket's first row, which the plain bar-click path uses).
