...
## 6. Empty-state placeholder when no source is loaded — P1

An idle window is currently just an empty grid. Either:

- Override `LogTableView::paintEvent` (`app/src/log_table_view.cpp`) and
  paint a hint when `model()->rowCount() == 0 && !IsSessionActive()`, or
- Stack a `QLabel` with `QStackedLayout` and switch on row count.

Suggested text:

> Drop a JSON Lines log here, press `Ctrl+O`, or open a stream with `Ctrl+Shift+O`.

Use `QPalette::PlaceholderText` so the color adapts to light/dark
automatically.
...
## 18. Keyboard discoverability — P3

- Add `actionShowShortcuts` (e.g. `Ctrl+/`) opening a `QDialog` that lists
  every shortcut, auto-discovered from
  `findChildren<QAction*>()` filtered by `!a->shortcut().isEmpty()`. The
  shortcuts in `app/src/main_window.ui` populate it for free.
- Give each `QAction` a `setStatusTip()` — Qt wires status-bar context
  display automatically on hover.
- For each action with a shortcut, also set its tool tip to
  `action->text() + " (" + action->shortcut().toString() + ")"` so toolbar
  buttons surface the shortcut.
...
for `Qt::FontRole` on chosen columns (message, raw JSON).
...
## 21. Small Qt 6 modernizations easy to miss — P3

- `Q_DECLARE_TR_FUNCTIONS` in non-`QObject` helpers instead of
  `QString::fromLatin1`.
- `QHeaderView::setSortIndicatorClearable(true)` (Qt 6.1+) lets users cycle
  Asc → Desc → none with one extra header click, which matters because
  "no sort" (chronological order) is meaningful here.
- `QLocale::system().toString(qint64)` for grouped digit separators
  ("12,345 lines" instead of "12345 lines") in the status bar.
- `QElapsedTimer` + a tiny formatter for "00:00:32 since start" in
  live-tail mode.
- `QApplication::setStyle(QStyleFactory::create("Fusion"))` for a uniform
